...
if __name__ == "__main__":
    ...
    difference_between_classes = np.count_nonzero(labels_train==0) - np.count_nonzero(labels_train==1)
    positive_indices = np.where(labels_train==1)
    tweets_train_positives = np.take(tweets_train, positive_indices[0]).tolist()

    if wise_augmentation==0:
        tweets_to_augment = np.random.choice(tweets_train_positives, difference_between_classes, replace=True).tolist()
        labels_to_augment = np.ones(difference_between_classes, dtype = np.int64)
        if augmentation_model == 'fasttext':
            tweets_augmented = augmentor.augmentation_word(tweets_to_augment, aug_p=augmentation_percentage)
        elif augmentation_model == 'spelling':
            tweets_augmented = augmentor.augmentation_spelling(tweets_to_augment, aug_p=augmentation_percentage)
        elif augmentation_model == 'gpt2':
            tweets_augmented = augmentor.augmentation_sentence(tweets_to_augment, model_type='gpt2')
        tweets_train = tweets_train + tweets_augmented
        labels_train = np.concatenate((labels_train,labels_to_augment))
    ...
    cluster_dir = root_address+ 'clusters_positives/'
    ...
    if load_clusters:
        ...
        cc = Clustering()
        embeddings_train = cc.get_embedding(tweets_train_positives)
    ...
    for i,cluster_num in enumerate(train_clusters):
        ...
        cluster_tweets.append((tweets_train_positives[i],1))
    ...
    for epoch in range(10):
        ...
        if wise_augmentation == 1:
            ...
            sampled_tweets_tuples =   sample_tweets_to_augment(cluster_num_to_tweets,clusters_to_augment,difference_between_classes)
